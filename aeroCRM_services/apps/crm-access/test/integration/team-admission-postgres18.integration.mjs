...
try {
	...
	assert.equal(retryRow.exchange, 'aerocrm.manual-retry');
	...
	const retryTransport = {
		publish: async (exchange, route, payload, options) => {
			...
			assert.equal(exchange, 'aerocrm.manual-retry');
	...
	assert.equal(converted.exchange, 'aerocrm.manual-retry');
