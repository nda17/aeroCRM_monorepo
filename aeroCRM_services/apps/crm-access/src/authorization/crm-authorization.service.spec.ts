...
describe('CRM service authorization', () => {
	...
	it.each(['OWNER', 'CRM_ADMIN'])(
		...
		async role => {
			...
			for (let i = 0; i < 2; i++)
				expect(
					await current.service.authorizeSource(
						workspaceId,
						'user-1'
					)
				).toMatchObject({
			...
			expect(current.identity.sourceContext).toHaveBeenCalledTimes(
				2
			);
	...
	it('rechecks CRM role and paid/Trial lifecycle for every native source authority', async () => {
		for (const role of ['OWNER', 'CRM_ADMIN'])
			await expect(
				setup(role, 'READ_ONLY').service.authorizeSource(
					workspaceId,
					'user-1'
				)
			).rejects.toBeInstanceOf(ForbiddenException);
		...
		current.identity.sourceContext.mockRejectedValue(
