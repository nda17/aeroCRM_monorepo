	...
	implements OnModuleInit, OnApplicationShutdown
{
	...
	async onModuleInit() {
		...
		const name = `aerocrm-crm-access-${this.runtime.role}`;
		if (
	...
	private async topology(channel: ConfirmChannel) {
		for (const [name, type] of [
			['aerocrm.events', 'topic'],
			['aerocrm.dead-letter', 'topic'],
			['aerocrm.manual-retry', 'direct']
		] as const)
		...
		for (const consumer of TEAM_CONSUMERS) {
			...
			await channel.bindQueue(queue, 'aerocrm.manual-retry', route);
