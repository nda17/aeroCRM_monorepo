...
const route = {
	AEROCRM_CHECKOUT: 'checkout',
	AEROCRM_SEAT_CHANGE: 'seats',
	AEROCRM_DISABLE_RENEWAL: 'renewal/disable',
	AEROCRM_CONFIRM_RENEWAL: 'renewal/confirm-price',
	AEROCRM_VERIFY_ORDER: 'orders/verify'
} as const;
...
export class CrmBillingCapacityService {
	...
	async canonicalOwner(workspaceId: string, subject: string) {
		const context = await this.identity.ownerContext(
			workspaceId,
			subject,
			getCrmAccessCorrelationId()
		);
	...
	async recover(
		...
		const op = await serializable(this.prisma, async tx => {
			...
			const commandType = 'AEROCRM_NOT_STARTED';
			await tx.crmTeamCommandReceipt.create({
				data: {
					commandId,
					workspaceId,
