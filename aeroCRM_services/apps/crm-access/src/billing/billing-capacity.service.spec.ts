...
function fixture() {
	...
	const identity = {
		...
		ownerContext: jest.fn().mockResolvedValue({
...
describe('CRM Billing durable capacity fence', () => {
	...
	it('reserves before HTTP, uses the admission lock namespace, and replays exact binding', async () => {
		const f = fixture(),
			command = checkout();
		const first = await f.service.prepare('AEROCRM_CHECKOUT', command);
		const second = await f.service.prepare('AEROCRM_CHECKOUT', command);
	...
	it('rejects lower than owner plus active roster and serializes competing operations', async () => {
		...
		f.state.members = 5;
		await expect(
			f.service.prepare('AEROCRM_CHECKOUT', checkout())
		).rejects.toBeInstanceOf(ConflictException);
		expect(f.tx.crmBillingOperation.create).not.toHaveBeenCalled();
		f.state.members = 1;
		await f.service.prepare('AEROCRM_CHECKOUT', checkout());
		await expect(
			f.service.prepare('AEROCRM_CHECKOUT', checkout())
		).rejects.toBeInstanceOf(ConflictException);
	});
	it('preserves the global Access command namespace across team and commerce', async () => {
		...
		await expect(
			f.service.prepare('AEROCRM_CHECKOUT', command)
		).rejects.toBeInstanceOf(ConflictException);
		expect(f.billing.request).not.toHaveBeenCalled();
	});
	it.each(['actor', 'workspace', 'body'])(
		...
		async field => {
			const f = fixture(),
				command = checkout();
			await f.service.prepare('AEROCRM_CHECKOUT', command);
			const changed = {
				...command,
				...(field === 'actor'
					? { actorSubject: 'other' }
			...
			await expect(
				f.service.prepare('AEROCRM_CHECKOUT', changed)
			).rejects.toBeInstanceOf(ConflictException);
		}
	);
	it('retains the pending ceiling on dependency failure, 404, and owner revocation', async () => {
		...
		f.identity.ownerContext.mockResolvedValue({
			ownerSubject: 'other',
			membership: { role: 'OWNER' }
		});
	...
	it('uses canonical sorted-key command hash including original actor', () => {
		...
		const hash = commerceHash('AEROCRM_CHECKOUT', command);
		expect(
			commerceHash(
				'AEROCRM_CHECKOUT',
