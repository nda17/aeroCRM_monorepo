...
export class CrmBillingController {
	...
	checkout(
		...
		return this.billing.command('AEROCRM_CHECKOUT', auth, body, key);
	}
	@Post('seats')
	@HttpCode(200)
	@Header('Cache-Control', 'no-store')
	seats(
		...
		return this.billing.command('AEROCRM_SEAT_CHANGE', auth, body, key);
	}
	...
	disable(
		...
		return this.billing.command('AEROCRM_DISABLE_RENEWAL', auth, body, key);
	}
	...
	confirm(
		...
		return this.billing.command('AEROCRM_CONFIRM_RENEWAL', auth, body, key);
	}
	@Get('orders/:id')
	@Header('Cache-Control', 'no-store')
	order(
	...
	verify(
		...
		return this.billing.command('AEROCRM_VERIFY_ORDER', auth, body, key);
	}
	@Get('history')
	@Header('Cache-Control', 'no-store')
	history(
