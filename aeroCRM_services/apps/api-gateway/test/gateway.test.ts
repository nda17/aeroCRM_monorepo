...
describe('API Gateway config', () => {
	...
	it('does not route removed Widgets and Telegram-auth paths', () => {
		const config = loadConfig({
			...baseEnv,
			GATEWAY_ROUTES_JSON: JSON.stringify([
				createTestRoute({ id: 'crm-intake', pathPrefix: '/api/v1/crm/intake' }),
				createTestRoute({ id: 'identity-auth', pathPrefix: '/api/v1/auth' })
			])
		});
		assert.equal(matchGatewayRoute('/api/v1/crm/intake/inbox', config.routes)?.id, 'crm-intake');
		for (const path of ['/api/v1/widgets', '/api/v1/widget', '/api/v1/quizzes', '/api/v1/telegram-auth']) {
			assert.equal(matchGatewayRoute(path, config.routes), undefined);
		}
	});
	...
	it('routes Billing settings with split public and admin auth policies', () => {
			...
			GATEWAY_ROUTES_JSON: JSON.stringify([
				{
					...
					pathPrefix: '/api/v1/billing-settings/crm/public',
		...
		const publicRoute = matchGatewayRoute(
			'/api/v1/billing-settings/crm/public',
			config.routes
		);
		const adminRoute = matchGatewayRoute(
			'/api/v1/billing-settings/admin/crm',
			config.routes
		);
		assert.deepEqual(
			publicRoute && {
		...
		assert.equal(
			matchGatewayRoute('/api/v1/billing-crm-settings', config.routes)
				?.id,
			undefined
		);
	});
	...
	it('matches the longest prefix only on an exact path boundary', () => {
		...
		assert.equal(
			matchGatewayRoute('/api/v1/crm/intake/42', routes)?.id,
			'crm-intake'
		);
		assert.equal(
			matchGatewayRoute('/api/v1/crm/42', routes)?.id,
			'crm'
		);
		assert.equal(
			matchGatewayRoute('/api/v1/crm-intake-extra', routes)?.id,
			undefined
		);
	});
...
describe('API Gateway proxy', () => {
	...
	it('preserves path, body, streaming, Set-Cookie and valid Authorization', async () => {
		...
		const result = await makeRequest(
			new URL('/api/v1/widget?key=value%202', gatewayUrl),
			{
				...
				headers: {
					...
					'x-auth-roles': 'ADMIN',
					'x-aerocrm-internal-token': 'spoofed-service-token',
		...
		assert.equal(proxied.headers['x-aerocrm-internal-token'], undefined);
	...
	it('never publishes internal service endpoints through the catch-all route', async () => {
		...
		for (const path of [
			...
			const result = await makeRequest(new URL(path, gatewayUrl), {
				headers: {
					'x-aerocrm-internal-token': 'attacker-controlled'
				}
			});
	...
	it('adds CORS only to generated responses using the exact origin policy', async () => {
		const allowed = await makeRequest(
			new URL('/api/v1/protected-route', gatewayUrl),
			{
				headers: {
					authorization: 'Bearer invalid',
					origin: 'https://aerocrm.test'
				}
			}
		);
