...
assert.match(
	parsed.pathname,
	/^\/aerocrm_notification_invitation_(?:test|ci)(?:_[a-z0-9]+)?$/
);
...
const kind = 'crm-invitation-email';
const eventType = 'notification.crm.invitation.email.requested.v1';
const eventIds = [];
let sends = 0;
...
const adapter = new NotificationDeliveryAdapterService(
	new EmailService({
		sendMail: async mail => {
			assert.equal(mail.subject, 'Приглашение в aeroCRM');
			assert.ok(
				mail.messageId.endsWith('.crm-invitation@aerocrm.space>')
			);
...
try {
	...
	assert.equal(outbox.routingKey, 'manual.crm-invitation-email');
	...
	assert.equal(outbox.routingKey, 'crm-invitation-email.dead-letter');
	...
	assert.ok(acks >= 8);
	console.log(
		'aeroCRM invitation PostgreSQL 18 integration: PASS (restricted roles, CHECKs, CAS/lease, concurrent dedup, skip, retry/DLQ/control, no outcome; fake SMTP only)'
	);
} finally {
...
function message(payload, routingKey = eventType, headers = {}) {
	return {
		...
		fields: { routingKey, exchange: 'aerocrm.events' },
...
async function taskReminderProof() {
	for (const channel of ['EMAIL', 'TELEGRAM']) {
		const reminderKind = `crm-task-reminder-${channel.toLowerCase()}`;
		const reminderType = `notification.crm.task-reminder.${channel.toLowerCase()}.requested.v1`;
		const reminder = {
			schemaVersion: 1,
			eventId: randomUUID(),
