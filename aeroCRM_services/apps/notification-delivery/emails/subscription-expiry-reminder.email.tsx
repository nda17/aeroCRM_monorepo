...
const SubscriptionExpiryReminderEmail = ({
	...
	return (
		<EmailLayout
			preview={
				isLastDay
					? 'Сегодня последний день вашей подписки aeroCRM'
					: `До окончания подписки aeroCRM осталось ${getDaysLabel(daysBeforeExpiry)}`
			}
			title={
				isLastDay
		...
		>
			<Text style={primaryTextStyle} className="ww-primary-text">
				{isLastDay
					? 'Сегодня последний день действия вашей подписки aeroCRM.'
					: `До окончания вашей подписки aeroCRM осталось ${getDaysLabel(daysBeforeExpiry)}.`}
			</Text>
