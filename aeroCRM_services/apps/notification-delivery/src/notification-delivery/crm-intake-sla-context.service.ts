...
@Injectable()
export class CrmIntakeSlaContextService implements OnModuleInit {
	constructor(
	...
	private kinds() {
		const configured =
			this.config
				...
				?.split(',')
				.map(kind => kind.trim()) ?? [];
		return CRM_INTAKE_SLA_KINDS.filter(kind =>
	...
	assertConfigured(requireBoth = false): void {
		...
		if (!kinds.length || (requireBoth && kinds.length !== 2))
			throw new Error('aeroCRM Intake SLA consumers are not enabled');
		this.configuration();
		if (
			kinds.includes('crm-intake-sla-email') &&
			['SMTP_SERVER', 'SMTP_LOGIN', 'SMTP_PASSWORD'].some(
				key => !this.config.get<string>(key)?.trim()
			)
		)
			throw new Error(
				'aeroCRM Intake SLA email transport is not configured'
			);
		if (kinds.includes('crm-intake-sla-telegram'))
	...
	async resolve(
		event: CrmIntakeSlaEventPayload
		...
		if (!this.kinds().includes(kind))
			throw new Error('aeroCRM Intake SLA channel is not enabled');
		...
		try {
			...
			abort.abort();
			throw Object.assign(
				new Error('aeroCRM Intake SLA context is unavailable'),
				{ code: 'ETIMEDOUT' }
			);
		} finally {
