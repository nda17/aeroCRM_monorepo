...
import {
	CRM_INTAKE_SLA_EMAIL_EVENT_TYPE as EMAIL,
	CRM_INTAKE_SLA_TELEGRAM_EVENT_TYPE as TELEGRAM,
...
import type { CrmIntakeSlaEventPayload } from '../messaging/delivery-event.types';
import {
	assertCrmIntakeSlaEvent,
	parseSlaDeliveryContext
} from '../messaging/crm-intake-sla.contract';
...
const event: CrmIntakeSlaEventPayload = {
	schemaVersion: 1,
...
function setupConfig(overrides: Record<string, string | undefined> = {}) {
	const values: Record<string, string | undefined> = {
		NOTIFICATION_DELIVERY_KINDS: CRM_INTAKE_SLA_KINDS.join(','),
	...
	return {
		config,
		service: new CrmIntakeSlaContextService(
			config,
...
describe('Intake SLA opaque broker and owner context contracts', () => {
	...
	it('retains six default kinds and opts into independent SLA channels only', () => {
		const defaults = parseNotificationDeliveryKinds(undefined);
		expect(defaults).toHaveLength(6);
		for (const kind of CRM_INTAKE_SLA_KINDS)
			expect(defaults).not.toContain(kind);
		expect(
			parseNotificationDeliveryKinds(CRM_INTAKE_SLA_KINDS.join(','))
		).toEqual(CRM_INTAKE_SLA_KINDS);
		for (const [kind, type] of [
			[CRM_INTAKE_SLA_KINDS[0], EMAIL],
			[CRM_INTAKE_SLA_KINDS[1], TELEGRAM]
		] as const) {
	...
	it.each([
		...
		extra =>
			expect(() =>
				assertCrmIntakeSlaEvent({ ...event, ...extra })
			).toThrow()
	);
	it('binds payload type, channel, AMQP message ID and routing key before consumer processing', () => {
		for (const extra of [
			{ messageId: otherId },
			{ eventType: TELEGRAM },
			{ kind: 'crm-intake-sla-telegram' as const },
			{ kind: 'crm-task-reminder-email' as const },
			{ routingKey: CRM_TASK_REMINDER_EMAIL_EVENT_TYPE }
		])
			expect(() =>
	...
	it('does not resolve an opted-out channel even when the other transport is configured', async () => {
		...
		const value = setupConfig({
			NOTIFICATION_DELIVERY_KINDS: 'crm-intake-sla-email',
			TELEGRAM_INFO_BOT_TOKEN: undefined
		});
	...
	it.each([
		...
		async configuredKinds => {
			...
			for (const kind of CRM_INTAKE_SLA_KINDS) {
				...
				for (const reminderKind of CRM_TASK_REMINDER_KINDS)
...
describe('Intake SLA adapter requires fresh authority and an active receipt lease', () => {
	...
	it('renders branded escaped email and fixed Intake deep-link only after matching live claim', async () => {
		...
		expect(value.sendMail).toHaveBeenCalledWith(
			expect.objectContaining({
				...
				subject: 'Обращение без ответа в aeroCRM',
				messageId: `<${event.eventId}.crm-intake-sla@aerocrm.space>`,
				html: expect.stringContaining(
					`href="https://workspace.aerocrm.space/inbox?entry=${context().content.entryId}"`
				)
			})
		);
		...
		expect(html).toContain('cid:aerocrm-notification-logo');
	...
	it('rejects unclaimed, channel-mismatched, wrong-id and foreign-type payloads before owner lookup', async () => {
		...
		for (const [kind, id, claim] of [
			['crm-intake-sla-email', event.eventId, undefined],
			['crm-intake-sla-telegram', event.eventId, 'claim'],
			['crm-intake-sla-email', otherId, 'claim']
		] as const)
			await expect(
...
describe('Private Intake SLA reader readiness', () => {
	...
	it('uses a private Intake-specific GET readiness route', () => {
		...
		expect(
			Reflect.getMetadata(
				PATH_METADATA,
				CrmIntakeSlaReadinessController.prototype.readiness
			)
		).toBe('readiness');
		expect(
			Reflect.getMetadata(
	...
	it('requires actual loopback and exact Intake reverse credentials, never Sales credentials', () => {
		const guard = new CrmIntakeSlaReadinessGuard(setupConfig().config);
		...
		for (const key of [
			...
			expect(() =>
				new CrmIntakeSlaReadinessGuard(
	...
	it('requires both configured SLA consumers and fresh worker/broker/outbox readiness', async () => {
		...
		const controller = new CrmIntakeSlaReadinessController(
			{
				getReadinessHealth
		...
		expect(isReadyForKinds).toHaveBeenCalledWith(CRM_INTAKE_SLA_KINDS);
		...
		const partial = new CrmIntakeSlaReadinessController(
			{
				getReadinessHealth
			} as unknown as NotificationDeliveryHealthService,
			setupConfig({
