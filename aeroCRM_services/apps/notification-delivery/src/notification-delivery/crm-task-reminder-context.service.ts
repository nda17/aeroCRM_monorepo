...
@Injectable()
export class CrmTaskReminderContextService implements OnModuleInit {
	constructor(
	...
	private kinds() {
		const configured =
			this.config
				...
				?.split(',')
				.map(kind => kind.trim()) ?? [];
		return CRM_TASK_REMINDER_KINDS.filter(kind =>
	...
	assertConfigured(requireBoth = false): void {
		...
		if (!kinds.length || (requireBoth && kinds.length !== 2))
			throw new Error('aeroCRM task reminder consumers are not enabled');
		this.configuration();
		if (
			kinds.includes('crm-task-reminder-email') &&
			['SMTP_SERVER', 'SMTP_LOGIN', 'SMTP_PASSWORD'].some(
				key => !this.config.get<string>(key)?.trim()
			)
		)
			throw new Error(
				'aeroCRM task reminder email transport is not configured'
			);
		if (kinds.includes('crm-task-reminder-telegram'))
	...
	async resolve(
		event: CrmTaskReminderEventPayload
		...
		if (!this.kinds().includes(kind))
			throw new Error('aeroCRM task reminder channel is not enabled');
		...
		try {
			...
			abort.abort();
			throw Object.assign(
				new Error('aeroCRM task reminder context is unavailable'),
				{ code: 'ETIMEDOUT' }
			);
		} finally {
