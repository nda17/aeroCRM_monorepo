...
import {
	CRM_TASK_REMINDER_EMAIL_EVENT_TYPE as EMAIL,
	CRM_TASK_REMINDER_TELEGRAM_EVENT_TYPE as TELEGRAM,
	CRM_TASK_REMINDER_KINDS
} from '../messaging/messaging.constants';
import type { CrmTaskReminderEventPayload } from '../messaging/delivery-event.types';
import {
	assertCrmTaskReminderEvent,
	parseReminderDeliveryContext
} from '../messaging/crm-task-reminder.contract';
...
const event: CrmTaskReminderEventPayload = {
	schemaVersion: 1,
...
function setupConfig(overrides: Record<string, string | undefined> = {}) {
	const values: Record<string, string | undefined> = {
		NOTIFICATION_DELIVERY_KINDS: CRM_TASK_REMINDER_KINDS.join(','),
	...
	return {
		config,
		service: new CrmTaskReminderContextService(
			config,
...
describe('Task reminder broker/context contract', () => {
	...
	it('keeps six default kinds and opts into independent email/Telegram routes', () => {
		expect(parseNotificationDeliveryKinds(undefined)).toHaveLength(6);
		expect(
			parseNotificationDeliveryKinds(CRM_TASK_REMINDER_KINDS.join(','))
		).toEqual(CRM_TASK_REMINDER_KINDS);
		for (const [kind, type] of [
			[CRM_TASK_REMINDER_KINDS[0], EMAIL],
			[CRM_TASK_REMINDER_KINDS[1], TELEGRAM]
		] as const)
	...
	it.each([
		...
		expect(() =>
			assertCrmTaskReminderEvent({ ...event, ...extra })
		).toThrow()
	);
	it('binds AMQP message ID, kind and event type', () => {
		for (const extra of [
			...
			{ eventType: TELEGRAM },
			{ kind: 'crm-task-reminder-telegram' as const }
		])
			expect(() =>
		...
		expect(fetchMock).toHaveBeenCalledWith(
			...
			expect.objectContaining({
				...
				headers: expect.objectContaining({
					'x-aerocrm-service': 'notification-delivery',
					'x-aerocrm-internal-token': fixtureToken
				}),
				body: JSON.stringify({
					schemaVersion: 1,
					eventId: event.eventId,
	...
	it.each([
		...
		async response => {
			...
			await expect(setupConfig().service.resolve(event)).rejects.toThrow(
				'aeroCRM task reminder context is unavailable'
			);
		}
	);
	it('sanitizes network errors and aborts timed out reads', async () => {
		...
		try {
			...
			).rejects.toThrow('aeroCRM task reminder context is unavailable');
...
describe('Task reminder adapter uses fresh context and current lease (fake transports)', () => {
	...
	it('renders escaped branded email with fixed task deep-link and stable message ID only after live claim', async () => {
		...
		expect(value.sendMail).toHaveBeenCalledWith(
			expect.objectContaining({
				...
				subject: 'Напоминание о задаче aeroCRM',
				messageId: `<${event.eventId}.crm-task-reminder@aerocrm.space>`,
				html: expect.stringContaining(
					`href="https://workspace.aerocrm.space/planner?task=${context().content.taskId}"`
				)
			})
		);
	...
	it('rejects channel/id/unclaimed commands before context lookup', async () => {
		...
		for (const [kind, id, claim] of [
			['crm-task-reminder-email', event.eventId, undefined],
			['crm-task-reminder-telegram', event.eventId, 'claim'],
			['crm-task-reminder-email', event.reference.id, 'claim']
		] as const)
			await expect(
...
describe('Private Sales reminder readiness', () => {
	it('uses the exact agreed GET endpoint (delivery-context uses POST separately)', () => {
		...
		expect(
			Reflect.getMetadata(
				PATH_METADATA,
				CrmTaskReminderReadinessController.prototype.readiness
			)
		).toBe('readiness');
		expect(
			Reflect.getMetadata(
	...
	it('requires exact private pair credentials and actual socket loopback', () => {
		const guard = new CrmTaskReminderReadinessGuard(
			setupConfig().config
		);
		...
		expect(() =>
			new CrmTaskReminderReadinessGuard(
				setupConfig({
					NOTIFICATION_DELIVERY_CRM_SALES_TOKEN: fixtureToken
				}).config
			).canActivate(request())
	...
	it('never claims readiness without both running consumers, worker/broker/outbox and transport configuration', async () => {
		...
		const controller = new CrmTaskReminderReadinessController(
			{
				getReadinessHealth
	...
	it('namespace migration preserves every unrelated CHECK clause from the immutable baseline', () => {
		const root = join(__dirname, '../../prisma/migrations');
		const baseline = readFileSync(
			join(root, '20260920000000_init_aerocrm/migration.sql'),
			'utf8'
		);
		const migration = readFileSync(
			join(root, '20260920010000_crm_runtime_contracts/migration.sql'),
			'utf8'
		);
		for (const name of [
			...
			const prior = baseline.match(pattern)?.[0];
			const current = migration.match(pattern)?.[0];
			expect(prior).toBeDefined();
			expect(current).toBe(prior?.replace(/wincrm/g, 'crm'));
		}
		expect(migration).toContain('IN ACCESS EXCLUSIVE MODE');
		expect(migration).toContain("RAISE EXCEPTION 'CRM contract cutover requires an empty legacy delivery ledger");
		expect(migration).not.toMatch(/(?:UPDATE|DELETE FROM)\s+"notification_delivery"/);
	});
});
