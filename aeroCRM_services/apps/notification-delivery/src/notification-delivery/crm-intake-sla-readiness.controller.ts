...
} from './crm-intake-sla-context.service';
...
@Injectable()
export class CrmIntakeSlaReadinessGuard implements CanActivate {
	...
	canActivate(context: ExecutionContext): boolean {
		...
		if (
			...
			request.headers['x-aerocrm-service'] !== 'crm-intake'
		)
...
@UseGuards(CrmIntakeSlaReadinessGuard)
export class CrmIntakeSlaReadinessController {
	constructor(
		private readonly health: NotificationDeliveryHealthService,
		private readonly reminder: CrmIntakeSlaContextService,
	...
	async readiness() {
		try {
			...
			this.reminder.assertConfigured(true);
			if (!this.worker.isReadyForKinds(CRM_INTAKE_SLA_KINDS))
