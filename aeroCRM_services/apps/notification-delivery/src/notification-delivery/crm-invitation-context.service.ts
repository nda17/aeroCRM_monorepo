...
import { isCrmInvitationDeliveryEnabled } from '../messaging/messaging.constants';
import type { CrmInvitationEmailRequestedEventPayload } from '../messaging/delivery-event.types';
import {
	hasExactInvitationKeys,
	isCanonicalInvitationDate,
	isNormalizedInvitationEmail
} from '../messaging/crm-invitation.contract';

const MAX_RESPONSE_BYTES = 4096;
const CONTEXT_TIMEOUT_MS = 5000;

@Injectable()
export class CrmInvitationContextService implements OnModuleInit {
	...
	private enabled(): boolean {
		return isCrmInvitationDeliveryEnabled(
	...
	private configuration(): { origin: string; token: string } {
		if (!this.enabled())
			throw new Error('aeroCRM invitation delivery is not enabled');
	...
	async canDeliver(
		event: CrmInvitationEmailRequestedEventPayload
	): Promise<boolean> {
		...
		try {
			const response = await fetch(
				`${origin}/internal/v1/notification-delivery/crm-invitations/${event.reference.id}/delivery-context`,
				{
					method: 'POST',
					redirect: 'error',
			...
			throw new Error('aeroCRM invitation eligibility is unavailable');
		} finally {
