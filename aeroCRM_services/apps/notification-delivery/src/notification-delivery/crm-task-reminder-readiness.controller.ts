...
} from './crm-task-reminder-context.service';
...
@Injectable()
export class CrmTaskReminderReadinessGuard implements CanActivate {
	...
	canActivate(context: ExecutionContext): boolean {
		...
		if (
			...
			request.headers['x-aerocrm-service'] !== 'crm-sales'
		)
		...
		const supplied = request.headers['x-aerocrm-internal-token'];
...
@UseGuards(CrmTaskReminderReadinessGuard)
export class CrmTaskReminderReadinessController {
	constructor(
		private readonly health: NotificationDeliveryHealthService,
		private readonly reminder: CrmTaskReminderContextService,
	...
	async readiness() {
		try {
			...
			this.reminder.assertConfigured(true);
			if (!this.worker.isReadyForKinds(CRM_TASK_REMINDER_KINDS))
