...
const event: CrmInvitationEmailRequestedEventPayload = {
	schemaVersion: 1,
...
function service(overrides: Record<string, string | undefined> = {}) {
	const values: Record<string, string | undefined> = {
		NOTIFICATION_DELIVERY_KINDS: 'crm-invitation-email',
		IDENTITY_INTERNAL_BASE_URL: 'http://127.0.0.1:4500',
		IDENTITY_NOTIFICATION_DELIVERY_TOKEN: fixtureToken,
		...overrides
	};
	return new CrmInvitationContextService({
		get: (key: string) => values[key]
	} as ConfigService);
}

describe('CrmInvitationContextService', () => {
	afterEach(() => {
	...
	it('uses only the scoped pair headers and exact stored event binding; no redirects', async () => {
		...
		expect(fetcher).toHaveBeenCalledWith(
			`http://127.0.0.1:4500/internal/v1/notification-delivery/crm-invitations/${event.reference.id}/delivery-context`,
	...
	it.each([
		...
		await expect(service().canDeliver(event)).rejects.toThrow(
			'aeroCRM invitation eligibility is unavailable'
		);
	});
