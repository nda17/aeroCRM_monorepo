...
import { NotificationDeliveryPrismaService } from '../notification-delivery/prisma/notification-delivery-prisma.service';
...
export class WorkspaceClosureInternalGuard implements CanActivate {
	...
	constructor(config: ConfigService) {
		const value =
			config.get<string>('NOTIFICATION_DELIVERY_CRM_ACCESS_TOKEN')?.trim() ||
			'';
		if (
			value.length < 32 ||
			value.length > 4096 ||
			...
			throw new Error(
				'NOTIFICATION_DELIVERY_CRM_ACCESS_TOKEN requires a distinct non-placeholder secret'
			);
...
export class WorkspaceClosureService {
	...
	async fence(binding: ClosureEnvelope) {
        ...
        INSERT INTO notification_delivery.workspace_closure_fences (workspace_id, closure_id, generation, owner_subject, requested_at, fenced_at)
		...
		return {
			...
			priorDispatchCount: await this.prisma.notificationDeliveryReceipt.count({
				where: {
					crmWorkspaceId: binding.workspaceId,
					crmDispatchStartedAt: { not: null }
				}
			})
		};
	}
