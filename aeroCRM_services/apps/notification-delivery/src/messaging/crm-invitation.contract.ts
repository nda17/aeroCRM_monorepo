import type { CrmInvitationEmailRequestedEventPayload } from './delivery-event.types';
import { CRM_INVITATION_EMAIL_EVENT_TYPE } from './messaging.constants';
...
export function assertCrmInvitationEvent(
	value: unknown
): asserts value is CrmInvitationEmailRequestedEventPayload {
	if (
		...
		value.eventType !== CRM_INVITATION_EMAIL_EVENT_TYPE ||
		...
		value.reference.type !== 'crm-invitation' ||
		...
		throw new Error('Invalid aeroCRM invitation notification contract');
	}
}
