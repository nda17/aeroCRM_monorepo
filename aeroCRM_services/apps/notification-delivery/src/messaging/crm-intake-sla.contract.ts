import type { CrmIntakeSlaEventPayload } from './delivery-event.types';
import {
...
export function assertCrmIntakeSlaEvent(
	value: unknown
): asserts value is CrmIntakeSlaEventPayload {
	if (
		!exact(value, [
			'schemaVersion',
			'eventId',
		...
		throw new Error('Invalid aeroCRM Intake SLA event');
}
export function parseSlaDeliveryContext(
	...
	const invalid = () => new Error('Invalid aeroCRM Intake SLA context');
	if (
		!exact(value, [
			'schemaVersion',
			'eventId',
