...
const assertOperationsBackupReportEvent = (
	payload: JsonRecord
): NotificationDeliveryKind => {
	assertExactKeys(payload, ['schemaVersion', 'eventType', 'eventId', 'occurredAt', 'reference', 'destination', 'content']);
	if (payload.schemaVersion !== 1 || payload.eventType !== OPERATIONS_BACKUP_REPORT_TELEGRAM_EVENT_TYPE) throw new Error('Invalid backup report contract');
	assertUuid(payload.eventId, 'payload.eventId');
	assertIsoDate(payload.occurredAt, 'payload.occurredAt');
	const reference = assertRecord(payload.reference, 'payload.reference');
	assertExactKeys(reference, ['type', 'id'], [], 'payload.reference');
	if (reference.type !== 'operations-backup-report') throw new Error('Invalid backup report reference');
	assertUuid(reference.id, 'payload.reference.id');
	const destination = assertRecord(payload.destination, 'payload.destination');
	assertExactKeys(destination, ['telegramChatId', 'messageThreadId'], [], 'payload.destination');
	assertString(destination.telegramChatId, 'payload.destination.telegramChatId');
	if (!Number.isInteger(destination.messageThreadId) || Number(destination.messageThreadId) < 1) throw new Error('Invalid backup report thread');
	const content = assertRecord(payload.content, 'payload.content');
	assertExactKeys(content, ['text', 'consoleUrl'], [], 'payload.content');
	assertString(content.text, 'payload.content.text', { maxLength: 10000 });
	assertString(content.consoleUrl, 'payload.content.consoleUrl', { maxLength: 2048 });
	const url = new URL(content.consoleUrl as string);
	if (url.protocol !== 'https:' || url.username || url.password || url.search || url.hash) throw new Error('Invalid private S3 console URL');
	return 'operations-backup-report-telegram';
};
...
const resolveExpectedKind = (
	...
	switch (payload.eventType) {
		...
		case OPERATIONS_BACKUP_REPORT_TELEGRAM_EVENT_TYPE:
			return assertOperationsBackupReportEvent(payload);
		case DAILY_SUMMARY_TELEGRAM_NOTIFICATION_EVENT_TYPE:
		case SUBSCRIPTION_EXPIRY_EMAIL_NOTIFICATION_EVENT_TYPE:
		case SUBSCRIPTION_EXPIRY_TELEGRAM_NOTIFICATION_EVENT_TYPE:
...
export function assertMessagingEventContract(
	...
	if (
		...
		throw new Error('Notification eventId must match the AMQP messageId');
	}
