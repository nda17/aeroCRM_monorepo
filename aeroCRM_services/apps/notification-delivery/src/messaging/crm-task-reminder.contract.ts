import type { CrmTaskReminderEventPayload } from './delivery-event.types';
import {
...
export function assertCrmTaskReminderEvent(
	value: unknown
): asserts value is CrmTaskReminderEventPayload {
	if (
		!exact(value, [
			'schemaVersion',
			'eventId',
		...
		value.reference.type !== 'crm-task-reminder' ||
		!uuid(value.reference.id) ||
		!uuid(value.reference.workspaceId)
	)
		throw new Error('Invalid aeroCRM task reminder event');
}
...
export function parseReminderDeliveryContext(
	...
	const invalid = () => new Error('Invalid aeroCRM task reminder context');
	if (
		!exact(value, [
			'schemaVersion',
			'eventId',
