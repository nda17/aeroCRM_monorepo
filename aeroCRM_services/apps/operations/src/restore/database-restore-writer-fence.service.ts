	...
	async apply(
		...
		await this.process.executeSql(
			...
			[
				...
				`ALTER ROLE ${this.identifier(target.adminRole)} SET ${this.identifier('aerocrm.restore_generation')} TO ${this.literal(generationMarker)};`,
				...roles.map(
...
$database_restore_verify_reapply_boundary$;`;
	...
	private verifyGenerationMarker(
		...
		WHERE role_state.rolname = ${this.literal(target.adminRole)}
			AND split_part(configured.setting, '=', 1) = 'aerocrm.restore_generation'
	) <> 1 OR NOT EXISTS (
		SELECT 1
		FROM pg_db_role_setting AS role_setting
		JOIN pg_roles AS role_state ON role_state.oid = role_setting.setrole
		...
		WHERE role_state.rolname = ${this.literal(target.adminRole)}
			...
			AND configured.setting = ${this.literal(`aerocrm.restore_generation=${generationMarker}`)}
	) THEN
