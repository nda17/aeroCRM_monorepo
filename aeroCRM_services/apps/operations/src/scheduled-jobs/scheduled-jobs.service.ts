...
export class ScheduledJobsService {
	...
	async claimBackup(
		...
		if (
			...
			throw new Error('Lease duration is invalid');
		if (!this.backupTarget(event.jobType) && event.jobType !== DATABASE_BACKUP_REPORT_JOB_TYPE) return { status: 'REJECT' };
		return this.prisma.$transaction(async transaction => {
	...
	async completeBackupReport(input: {
		id: string; leaseToken: string; periodStart: Date; chatId: string;
		messageThreadId: number; text: string; consoleUrl: string;
		result: Prisma.InputJsonValue;
	}): Promise<boolean> {
		return this.prisma.$transaction(async transaction => {
			const updated = await transaction.scheduledJobRun.updateMany({
				where: { id: input.id, jobType: DATABASE_BACKUP_REPORT_JOB_TYPE,
					status: ScheduledJobRunStatus.PROCESSING, leaseToken: input.leaseToken,
					leaseExpiresAt: { gt: new Date() } },
				data: { status: ScheduledJobRunStatus.SUCCEEDED, result: input.result,
					finishedAt: new Date(), leaseOwner: null, leaseToken: null,
			...
			await this.outbox.enqueue(transaction, {
				eventId, deduplicationKey: `backup-report:${input.id}:telegram`,
				eventType, aggregateType: 'database-backup-report', aggregateId: input.id,
				routingKey: eventType,
				headers: { 'x-aerocrm-service': 'operations' },
				payload: { schemaVersion: 1, eventType, eventId,
					occurredAt: new Date().toISOString(),
					reference: { type: 'operations-backup-report', id: input.id },
					destination: { telegramChatId: input.chatId, messageThreadId: input.messageThreadId },
					content: { text: input.text, consoleUrl: input.consoleUrl } }
			});
			await transaction.telegramBotSettings.update({
				where: { id: 'singleton' },
				data: { databaseBackupLastSentPeriodStart: input.periodStart,
