...
import { DatabaseBackupS3Service } from './maintenance/database-backup-s3.service';
...
const RESTORE_ENABLED = process.env.DATABASE_RESTORE_ENABLED === 'true';
if (RESTORE_ENABLED) {
	throw new Error('S3 restore admission and single-cluster ACL are not available; DATABASE_RESTORE_ENABLED must remain false');
}
const API_CONTROLLERS =
	PROCESS_ROLE === 'api'
		? [
				...
				...(RESTORE_ENABLED ? [DatabaseRestoreController] : []),
