...
function fixture() {
	...
	const backup = {
		createAndStore: jest
			.fn()
			.mockResolvedValue({ storage: 'S3', fileSize: 1024 })
	};
	const prisma = {
	...
	const service = new MaintenanceWorkerService(
		...
		prisma as never,
		alerts as never,
		{ get: jest.fn().mockReturnValue('https://console.example.invalid/bucket') } as never
	);
	return {
		service,
		jobs,
...
describe('MaintenanceWorkerService durable backup outcomes', () => {
	...
	it.each([
		...
		async (status, decision) => {
			...
			expect(f.backup.createAndStore).not.toHaveBeenCalled();
	...
	it('acknowledges an externally completed backup only after the leased completion succeeds', async () => {
		...
		expect(f.backup.createAndStore).toHaveBeenCalledWith(
			f.job.id,
			'operations',
	...
	it.each([
		...
		async (result, decision) => {
			const f = fixture();
			f.backup.createAndStore.mockRejectedValue(new Error('Backup failed'));
			if (result === 'throw')
	...
	it.each(['lost', 'unavailable'] as const)(
		...
		async mode => {
			...
			f.backup.createAndStore.mockImplementation(
