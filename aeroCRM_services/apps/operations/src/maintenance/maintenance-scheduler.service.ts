	...
	implements OnModuleInit, OnApplicationShutdown
{
	...
	async tick(now = new Date()): Promise<void> {
		...
		try {
			...
			const lastDelay = Math.max(...Object.values(DATABASE_BACKUP_DELAY_MINUTES));
			const reportDelay = hour * 60 + minute + lastDelay + 30;
			const reportPeriod = reportDelay >= 24 * 60
				? this.moscowPeriod(new Date(today.start.getTime() - 1))
				: today;
			const reportFor = new Date(reportPeriod.start.getTime() + reportDelay * 60_000);
			if (reportFor <= now) {
				await this.jobs.enqueueUnique({
					jobType: DATABASE_BACKUP_REPORT_JOB_TYPE,
					scheduleKey: `daily:backup-report:${reportPeriod.key}`,
					trigger: ScheduledJobRunTrigger.SCHEDULED,
					scheduledFor: reportFor,
