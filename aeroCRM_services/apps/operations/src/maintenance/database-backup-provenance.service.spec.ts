...
const evidence = (): DatabaseBackupProvenanceEvidence => ({
	...
	schema: 'reporting',
	fileName: 'aerocrm-reporting-db-2026-08-31T10-01-00-000Z.dump',
	fileSize: 1024,
...
describe('DatabaseBackupProvenanceService', () => {
	...
	it.each(DATABASE_BACKUP_PROVENANCE_TARGETS)(
		'preserves canonical bytes and signature for %s',
		async target => {
			const schema = target.replace(/-/g, '_');
			const input = {
				...evidence(),
				target,
				databaseName: `aerocrm_${schema}`,
				schema,
				fileName: `aerocrm-${target}-db-2026-08-31T10-01-00-000Z.dump`
			};
			const legacyCanonical = JSON.stringify({
				domain: 'aerocrm.operations.database-backup-provenance.v1',
			...
			expect(signed.signatureEd25519Base64).toBe(
				signEd25519(
					...
					Buffer.concat([
						Buffer.from(
							'aerocrm.operations.database-backup-provenance.v1'
						),
						Buffer.from([0]),
	...
	it.each([
		...
		[
			...
			{ ...evidence(), databaseName: 'aerocrm_campaigns' }
		],
		[
			'an unsafe artifact size',
			{ ...evidence(), fileSize: Number.MAX_SAFE_INTEGER + 1 }
		],
		[
