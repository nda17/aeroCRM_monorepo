...
signature с domain `aerocrm.operations.database-backup-provenance.v1`.
...
`aerocrm.operations.database-restore.v1`. Transient ошибка или занятый CAS
не создают hot-loop в main queue: исходный JSON `Buffer` публикуется с
publisher confirm и `mandatory` return-check в `aerocrm.retry` по exact
...
`operations-restore-worker` помечен `com.aerocrm.singleton=true` и имеет
...
`aerocrm.ru_infra/scripts/run-isolated-restore-rehearsal.sh` в отдельном
