...
const identity = createServer((request, response) => {
	if (
		request.headers['x-aerocrm-service'] !== 'campaigns' ||
		request.headers['x-aerocrm-internal-token'] !== identityToken
	) {
...
const billing = createServer((request, response) => {
	if (
		request.headers['x-aerocrm-service'] !== 'campaigns' ||
		request.headers['x-aerocrm-internal-token'] !== billingToken
	) {
		response.writeHead(403).end();
		return;
	}
...
async function runRabbitMqSmoke(
	...
	try {
		...
		try {
			...
			try {
				for (const queue of [
					'aerocrm.campaigns.snapshot',
					'aerocrm.campaigns.delivery-outcome.v2'
				]) {
			...
			try {
				await forbiddenChannel.checkQueue(
					'aerocrm.maintenance.database-backup'
				);
				throw new Error(
