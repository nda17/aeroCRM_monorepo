...
CREATE TABLE "crm_customers"."workspace_closure_fences" (
...
CREATE FUNCTION "crm_customers"."workspace_closure_fence_immutable"() RETURNS trigger
...
END;
$$;
CREATE TRIGGER workspace_closure_fence_immutable BEFORE UPDATE OR DELETE ON "crm_customers"."workspace_closure_fences"
FOR EACH ROW EXECUTE FUNCTION "crm_customers"."workspace_closure_fence_immutable"();

CREATE FUNCTION "crm_customers"."assert_workspace_open"(target_workspace uuid) RETURNS void
LANGUAGE plpgsql SET search_path = pg_catalog, crm_customers AS $$
DECLARE admitted uuid;
BEGIN
  INSERT INTO "crm_customers"."workspace_closure_fences" (workspace_id)
...
CREATE FUNCTION "crm_customers"."guard_workspace_business_write"() RETURNS trigger
LANGUAGE plpgsql SET search_path = pg_catalog, crm_customers AS $$
...
BEGIN
  ...
  END IF;
  PERFORM "crm_customers"."assert_workspace_open"(target_workspace);
  IF TG_OP = 'DELETE' THEN RETURN OLD; END IF;
  RETURN NEW;
END;
$$;
CREATE TRIGGER guard_workspace_closure BEFORE INSERT OR UPDATE OR DELETE ON "crm_customers"."companies" FOR EACH ROW EXECUTE FUNCTION "crm_customers"."guard_workspace_business_write"();
CREATE TRIGGER guard_workspace_closure BEFORE INSERT OR UPDATE OR DELETE ON "crm_customers"."contacts" FOR EACH ROW EXECUTE FUNCTION "crm_customers"."guard_workspace_business_write"();
CREATE TRIGGER guard_workspace_closure BEFORE INSERT ON "crm_customers"."intake_operation_slots" FOR EACH ROW EXECUTE FUNCTION "crm_customers"."guard_workspace_business_write"();

REVOKE ALL ON FUNCTION crm_customers.workspace_closure_fence_immutable() FROM PUBLIC;
REVOKE ALL ON FUNCTION crm_customers.assert_workspace_open(uuid) FROM PUBLIC;
REVOKE ALL ON FUNCTION crm_customers.guard_workspace_business_write() FROM PUBLIC;
