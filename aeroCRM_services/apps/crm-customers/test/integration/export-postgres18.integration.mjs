...
const local = value => {
	...
	assert.match(url.pathname, /^\/aerocrm_crm_customers_test[a-z0-9_]*$/);
...
try {
	...
	assert.equal(exportHeaders(v1Companies)['X-CRM-Export-Schema'], '1');
	...
	assert.equal(exportHeaders(v2Companies)['X-CRM-Export-Schema'], '2');
	...
	assert.equal(exportHeaders(v2Csv)['X-CRM-Export-Schema'], '2');
	assert.ok(
		v2Csv.body
			.toString()
			.startsWith(
