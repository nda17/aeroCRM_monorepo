...
const COMMAND_TYPE = 'UPDATE_AEROCRM_COMMERCIAL_POLICY';
...
export async function requireCrmCommercialPolicy(
	...
	if (!policy) {
		throw new ServiceUnavailableException({
			...
			message: 'Настройки aeroCRM временно недоступны'
		});
	}
	return policy;
}
	...
	async update(
		...
		if (!actorRole) {
			throw new ForbiddenException(
				'Настройки aeroCRM могут изменять ADMIN и DEV'
			);
		}
				...
				return await this.prisma.$transaction(
					async transaction => {
						...
						if (current.version >= 2_147_483_647) {
							throw new ServiceUnavailableException(
								'aeroCRM policy version limit reached'
							);
						}
						...
						await enqueueBillingAdminAudit(transaction, {
							...
							description: 'Обновлены цены и лимиты сотрудников aeroCRM',
							entity: {
		...
		throw new Error('aeroCRM policy update retry loop exhausted');
	}
}
