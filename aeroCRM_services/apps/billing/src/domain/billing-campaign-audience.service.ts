				...
				async transaction => {
					...
					for (;;) {
						const cursorFilter: Prisma.Sql = cursor
							? Prisma.sql`AND account.owner_subject > ${cursor}`
							: Prisma.empty;
						const rows: ActiveSubscriberRow[] =
							await transaction.$queryRaw<
								ActiveSubscriberRow[]
							>(Prisma.sql`
							SELECT DISTINCT account.owner_subject AS "userId"
							FROM billing.crm_commerce_accounts AS account
							JOIN billing.crm_entitlements AS entitlement
								ON entitlement.workspace_id = account.workspace_id
							WHERE entitlement.product_code = 'AEROCRM'
								AND entitlement.plan_code = 'PAID'
								AND entitlement.status = 'ACTIVE'::billing."CrmEntitlementStatus"
								AND entitlement.effective_until > ${asOf}
								${cursorFilter}
							ORDER BY "userId" ASC
