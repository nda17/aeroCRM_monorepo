...
export interface CrmPeriodPriceSnapshot {
...
export interface CrmSeatDurationInput {
	readonly nowMs: number;
	readonly newTotalSeats: number;
	readonly period: {
		readonly kind: 'PAID';
		readonly status: 'ACTIVE';
		readonly cycle: CrmBillingCycle;
		readonly startsAtMs: number;
		readonly expiresAtMs: number;
		readonly totalSeats: number;
		readonly priceSnapshot: CrmPeriodPriceSnapshot;
	};
}

export interface CrmSeatDurationResult {
	readonly cycle: CrmBillingCycle;
...
export type CrmSeatDurationErrorCode =
	| 'INVALID_INPUT'
	| 'INVALID_PAID_PERIOD'
	| 'INVALID_TIMESTAMP'
	| 'INVALID_SEAT_COUNT'
...
export class CrmSeatDurationError extends Error {
	constructor(readonly code: CrmSeatDurationErrorCode) {
		super(`Invalid aeroCRM seat-duration calculation: ${code}`);
		this.name = 'CrmSeatDurationError';
	}
}
...
function timestamp(value: unknown): bigint {
	if (typeof value !== 'number' || !Number.isSafeInteger(value)) {
		throw new CrmSeatDurationError('INVALID_TIMESTAMP');
	}
 ...
 */
export function calculateCrmSeatDuration(
	input: CrmSeatDurationInput
): CrmSeatDurationResult {
	if (!object(input) || !object(input.period)) {
		throw new CrmSeatDurationError('INVALID_INPUT');
	}
	const { period } = input;
	if (period.kind !== 'PAID' || period.status !== 'ACTIVE') {
		throw new CrmSeatDurationError('INVALID_PAID_PERIOD');
	}
	...
	if (
		...
		throw new CrmSeatDurationError('INVALID_PRICE_SNAPSHOT');
	}
	if (
		!integer(period.totalSeats, snapshot.includedSeats) ||
		!integer(input.newTotalSeats, snapshot.includedSeats)
	) {
		throw new CrmSeatDurationError('INVALID_SEAT_COUNT');
	}
	...
	if (newRemaining === 0n) {
		throw new CrmSeatDurationError('ZERO_REMAINING_DURATION');
	}
	if (oldRemaining > MAX_SAFE_INTEGER || newRemaining > MAX_SAFE_INTEGER) {
		throw new CrmSeatDurationError('DURATION_OUT_OF_RANGE');
	}
	const newExpiresAt = now + newRemaining;
	if (newExpiresAt < -MAX_DATE_MS || newExpiresAt > MAX_DATE_MS) {
		throw new CrmSeatDurationError('EXPIRY_OUT_OF_RANGE');
	}
	return {
		cycle: period.cycle,
