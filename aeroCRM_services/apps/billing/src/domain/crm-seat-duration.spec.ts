import {
	calculateCrmSeatDuration,
	CrmSeatDurationError,
	type CrmSeatDurationErrorCode,
	type CrmSeatDurationInput
} from './crm-seat-duration';

const DAY_MS = 86_400_000;
const NOW = 1_800_000_000_000;
const MAX_DATE_MS = 8_640_000_000_000_000;
const input = (): CrmSeatDurationInput => ({
	nowMs: NOW,
	newTotalSeats: 3,
	period: {
		kind: 'PAID',
...
function expectCode(value: unknown, code: CrmSeatDurationErrorCode) {
	expect(() =>
		calculateCrmSeatDuration(value as CrmSeatDurationInput)
	).toThrow(new CrmSeatDurationError(code));
}

describe('aeroCRM paid seat-duration conversion', () => {
	it.each([
		{
		...
		row => {
			const value = input();
			const result = calculateCrmSeatDuration({
				...value,
			...
			for (const oldSeats of [2, 3, 7, 100]) {
				for (const newSeats of [2, 3, 7, 100]) {
					for (const remaining of [10_001, DAY_MS + 1, 30 * DAY_MS]) {
						...
						const converted = calculateCrmSeatDuration(original);
						expect(
						...
						const reversed = calculateCrmSeatDuration({
							...original,
	...
	it('is deterministic, reads no ambient clock, and does not mutate snapshot metadata', () => {
		...
		try {
			expect(calculateCrmSeatDuration(frozen)).toEqual(
				calculateCrmSeatDuration(frozen)
			);
	...
	it('allows the inclusive JavaScript Date maximum but rejects extension beyond it', () => {
		...
		expect(calculateCrmSeatDuration(boundary).expiresAtMs).toBe(
			MAX_DATE_MS
		);
		expectCode(
