...
const operation = (changes: Record<string, unknown> = {}) => ({
	...
	requestSnapshot: {
		...
		returnUrl: `https://workspace.aerocrm.space/billing/return?workspaceId=${W}&orderId=${O}`,
...
function fixture(current = operation()) {
	...
	return {
		tx,
		prisma,
		service: new CrmCommerceService(
			prisma as never,
			{
...
describe('CrmCommerce deterministic contracts', () => {
	it.each([
		['MONTHLY', 2, 99000n],
		['MONTHLY', 3, 128000n],
		['YEARLY', 2, 990000n],
		...
		expect(crmPrice(snapshot, cycle as 'MONTHLY', seats)).toBe(amount)
	);
	it.each([0, 1, 10001, 2.5, NaN, Infinity])(
		'rejects invalid seats %s',
		seats =>
			expect(() => crmPrice(snapshot, 'MONTHLY', seats)).toThrow()
	);
	it.each([
		null,
		[],
		...
		expect(() => readCrmPriceSnapshot(value)).toThrow()
	);
	...
	it.each([
		...
		expect(
			crmPeriodEnd(new Date(date), cycle as 'MONTHLY').toISOString()
		).toBe(result)
	);
	it('rejects date overflow', () =>
		expect(() =>
			crmPeriodEnd(new Date(8640000000000000), 'YEARLY')
		).toThrow());
	it.each([0n, -1n, 1000000000001n])('rejects invalid money %s', value =>
		expect(() => crmDecimal(value)).toThrow()
	);
	it('formats minor units without floating point', () => {
		expect(crmDecimal(1n)).toBe('0.01');
		expect(crmDecimal(1000000000000n)).toBe('10000000000.00');
	});
	it.each(['\ud800', '\udc00', '\ufffd', '\u0000', ['RUB'], {}])(
	...
	it('hashes exact actor-bound user request, ignoring only the separately bound capacity fence', () => {
		...
		const hash = crmCommerceRequestHash(
			'AEROCRM_CHECKOUT',
			command as never
		);
		expect(
			crmCommerceRequestHash('AEROCRM_CHECKOUT', {
				...command,
				capacityFence: fence
			} as never)
		).toBe(hash);
		...
		expect(
			crmCommerceRequestHash('AEROCRM_SEAT_CHANGE', command as never)
		).not.toBe(hash);
	});
});

describe('CrmCommerce provider safety', () => {
	let flag: string | undefined;
	beforeEach(() => {
		flag = process.env.BILLING_CRM_PAYMENTS_ENABLED;
	...
	afterEach(() => {
		...
		else process.env.BILLING_CRM_PAYMENTS_ENABLED = flag;
		jest.useRealTimers();
	});
	it('takes a durable receipt and lease before any external provider call', async () => {
		...
		await expect(
			f.service.claimProviderOperation({
				schemaVersion: 1,
				eventType: 'billing.crm.provider-operation.requested.v1',
				eventId: E,
				operationId: P
			})
	...
	it('keeps original return URL across environment changes without reading a mutable renewal', async () => {
		const f = fixture(),
			old = process.env.CRM_FRONTEND_ORIGIN;
		try {
			process.env.CRM_FRONTEND_ORIGIN =
				'https://workspace.aerocrm.space';
			...
			if (old === undefined)
				delete process.env.CRM_FRONTEND_ORIGIN;
			else process.env.CRM_FRONTEND_ORIGIN = old;
		}
	});
	...
	it('captures immutable CREATE fields in the same transaction as its Outbox', async () => {
		...
		const previousOrigin = process.env.CRM_FRONTEND_ORIGIN;
		try {
			process.env.CRM_FRONTEND_ORIGIN = 'https://workspace.aerocrm.space';
			...
			if (previousOrigin === undefined) delete process.env.CRM_FRONTEND_ORIGIN;
			else process.env.CRM_FRONTEND_ORIGIN = previousOrigin;
		}
	...
	it.each([
		...
		code =>
			expect(new CrmProviderResponseError(code)).toMatchObject({
				code,
				retryable: false
			})
	);
	it.each([
		...
		async value => {
			...
			).rejects.toBeInstanceOf(CrmProviderResponseError);
	...
	it('first-dispatch evidence keeps a crash/redelivery authorization failure UNKNOWN and creates an actual DLQ Outbox', async () => {
		...
		expect(f.tx.outboxEvent.create).toHaveBeenCalledWith(
			expect.objectContaining({
				data: expect.objectContaining({
					exchange: 'aerocrm.billing.crm-provider.dead-letter',
...
describe('aeroCRM paid entitlement provenance', () => {
	...
	it.each([
		...
		async (date, status) => {
			...
			const base = {
				...
				provisioningCommandType: 'ACTIVATE_AEROCRM_TRIAL',
