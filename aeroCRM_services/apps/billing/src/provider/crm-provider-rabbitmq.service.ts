...
@Injectable()
export class CrmProviderRabbitMqService
	implements OnModuleInit, OnApplicationShutdown
{
	...
	async onModuleInit() {
		if (!crmProviderMessagingEnabled() || !this.runtime.workerEnabled)
			return;
		const configuration = crmProviderBrokerConfiguration();
		this.connection = connect([configuration.url], {
			heartbeatIntervalInSeconds: 10,
			reconnectTimeInSeconds: 5,
			connectionOptions: {
				clientProperties: {
					connection_name: 'aerocrm-billing-crm-provider-worker'
				}
			}
		});
		...
		this.channel = this.connection.createChannel({
			name: 'aerocrm-billing-crm-provider-worker',
			confirm: true,
			setup: async (channel: ConfirmChannel) => {
				if (!configuration.assertTopology) return;
				// aerocrm.events is provisioned by the broker owner, not by this credential.
				await channel.assertExchange(
					AEROCRM_PROVIDER_DEAD_EXCHANGE,
					'direct',
					{ durable: true }
				);
				await channel.assertQueue(AEROCRM_PROVIDER_QUEUE, {
					durable: true
				});
				...
				await channel.assertQueue(AEROCRM_PROVIDER_DEAD_QUEUE, {
					durable: true
				});
				await channel.bindQueue(
					AEROCRM_PROVIDER_DEAD_QUEUE,
		...
		try {
			...
			throw new Error('aeroCRM provider messaging initialization failed');
		} finally {
	...
	async consume(handler: (message: ConsumeMessage) => Promise<void>) {
		if (!this.channel || this.stopping)
			throw new Error('aeroCRM provider consumer is unavailable');
	...
	async requeue(message: ConsumeMessage) {
		if (!this.stopping)
			await new Promise<void>(resolve => {
				...
				const timer = setTimeout(finish, AEROCRM_PROVIDER_REQUEUE_MS);
