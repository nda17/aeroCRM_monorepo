...
export class YooKassaService {
	...
	async createPayment(
		...
		return this.request('/payments', {
			method: 'POST',
			rejectRedirects: input.productCode === 'AEROCRM',
	...
	getPayment(
		...
		return this.request(
			...
			{
				method: 'GET',
				rejectRedirects: productCode === 'AEROCRM'
			}
		);
	}
	...
	private credentials(): { shopId: string; secret: string } {
		const { shopId, secret } = this.credentialValues();
		if (!shopId || !secret) {
			throw new Error('YooKassa credentials are missing for selected launch mode');
		}
	...
	private credentialValues(): {
		...
		const shopId = (mode === 'test'
			? process.env.YOOKASSA_TEST_SHOP_ID
			: process.env.YOOKASSA_PRODUCTION_SHOP_ID)?.trim();
		const secret = (mode === 'test'
			? process.env.YOOKASSA_TEST_SECRET_KEY
			: process.env.YOOKASSA_PRODUCTION_SECRET_KEY)?.trim();
		return { shopId, secret };
	}

	private launchMode(): 'test' | 'production' {
		const mode = process.env.CRM_PAYMENT_LAUNCH_MODE;
		if (mode !== 'test' && mode !== 'production') {
			throw new Error('CRM_PAYMENT_LAUNCH_MODE must be test or production');
		}
		return mode;
	}
}
