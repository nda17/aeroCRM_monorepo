...
import type {
	...
	CrmProviderEvent,
	CrmProviderFailure
} from '../domain/crm-commerce.contract';
import { CrmProviderResponseError } from '../domain/crm-commerce.contract';
...
import {
	...
	CrmProviderAuthorizationError
} from './crm-access-authorization.client';
...
const POISON_CONSUMER = 'billing.crm-provider-invalid.v1';

export function parseCrmProviderMessage(
	...
	if (
		...
		event.eventType !== AEROCRM_PROVIDER_EVENT ||
		typeof event.eventId !== 'string' ||
		!UUID_V4.test(event.eventId) ||
		typeof event.operationId !== 'string' ||
		!UUID_V4.test(event.operationId) ||
		message.properties.messageId !== event.eventId ||
		message.properties.type !== AEROCRM_PROVIDER_EVENT ||
		message.fields.exchange !== 'aerocrm.events' ||
		message.fields.routingKey !== AEROCRM_PROVIDER_EVENT
	)
		throw new Error('INVALID_EVENT');
	return event as unknown as CrmProviderEvent;
}

@Injectable()
export class CrmProviderWorkerService
	implements OnModuleInit, OnApplicationShutdown
{
	private stopping = false;
	constructor(
		private readonly runtime: BillingRuntimeService,
		private readonly rabbit: CrmProviderRabbitMqService,
		...
		private readonly authorization: CrmAccessAuthorizationClient,
		private readonly prisma: BillingPrismaService
	) {}

	async onModuleInit() {
		if (!crmProviderMessagingEnabled() || !this.runtime.workerEnabled)
			return;
	...
	async handle(message: ConsumeMessage): Promise<void> {
		let event: CrmProviderEvent;
		try {
			event = parseCrmProviderMessage(message);
		} catch {
	...
	private async execute(claim: CrmProviderClaim) {
		let providerCallStarted = false;
		let providerPaymentId: string | undefined;
		let prepared: CrmPreparedProviderOperation = { action: 'SKIP' };
		try {
	...
	private classifyFailure(
		...
		if (error instanceof CrmProviderAuthorizationError)
			return {
				code: error.code,
				ambiguous: false,
		...
		await this.prisma.$transaction(
			async transaction => {
				await transaction.$executeRaw(
					Prisma.sql`SELECT pg_advisory_xact_lock(hashtextextended(${`billing-crm-provider-invalid:${hash}`}, 0))`
				);
				const prior =
				...
				const safeEvidence = {
					schemaVersion: 1,
					eventType: AEROCRM_PROVIDER_EVENT,
					eventId,
					code: 'INVALID_EVENT',
					contentSha256: hash
				};
				await transaction.integrationDeliveryFailure.create({
					data: {
						...
						routingKey: AEROCRM_PROVIDER_EVENT,
						payload: safeEvidence,
						errorCode: 'INVALID_EVENT',
						errorSafe: 'aeroCRM provider message contract is invalid',
						retryable: false
					}
				});
				await transaction.outboxEvent.create({
					data: {
						...
						messageId: eventId,
						eventType: AEROCRM_PROVIDER_EVENT,
						aggregateType: 'billing.crm-provider-invalid',
						aggregateId: eventId,
						exchange: AEROCRM_PROVIDER_DEAD_EXCHANGE,
						routingKey: AEROCRM_PROVIDER_EVENT,
						payload: safeEvidence,
						deduplicationKey: `crm-provider-invalid:${hash}`
					}
				});
