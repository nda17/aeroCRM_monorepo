import { CrmAccessAuthorizationClient } from './crm-access-authorization.client';
import type { BillingRuntimeService } from '../runtime/billing-runtime.service';

describe('aeroCRM provider fresh Access authorization', () => {
	...
	it('requires exact actor/workspace/operation/fence bindings with no redirects or user JWT', async () => {
		...
		await new CrmAccessAuthorizationClient(runtime).authorize(input);
	...
	it.each([
		...
		await expect(
			new CrmAccessAuthorizationClient(runtime).authorize(input)
	...
	it('bounds response bytes and hides dependency error content', async () => {
		...
		await expect(
			new CrmAccessAuthorizationClient(runtime).authorize(input)
		).rejects.toThrow('aeroCRM payment authorization is unavailable');
	});
	...
	it.each([
		...
		expect(() => new CrmAccessAuthorizationClient(runtime)).toThrow(
	...
	it('does not require new credentials when the feature is disabled or this is not a worker', () => {
		...
		expect(
			() => new CrmAccessAuthorizationClient(runtime)
		).not.toThrow();
		process.env.BILLING_CRM_PAYMENTS_ENABLED = 'true';
		expect(
			() =>
				new CrmAccessAuthorizationClient({
					workerEnabled: false
