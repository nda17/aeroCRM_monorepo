...
import type {
	CrmCapacityFence,
	CrmProviderFailureCode
} from '../domain/crm-commerce.contract';
import { BillingRuntimeService } from '../runtime/billing-runtime.service';
import { crmPaymentsEnabled } from './crm-provider.config';

export class CrmProviderAuthorizationError extends Error {
	constructor(readonly code: CrmProviderFailureCode) {
		super(
			code === 'AUTHORIZATION_REVOKED'
				? 'aeroCRM payment authorization was revoked'
				: 'aeroCRM payment authorization is unavailable'
		);
	}
}

export interface CrmProviderAuthorizationInput {
	workspaceId: string;
	ownerSubject: string;
	commandId: string;
...
@Injectable()
export class CrmAccessAuthorizationClient {
	...
	constructor(runtime: BillingRuntimeService) {
		this.origin = '';
		this.token = '';
		if (!crmPaymentsEnabled() || !runtime.workerEnabled) return;
		const raw =
	...
	async authorize(input: CrmProviderAuthorizationInput): Promise<void> {
		if (!this.origin || !this.token)
			throw new CrmProviderAuthorizationError('DEPENDENCY_UNAVAILABLE');
		const body = {
			schemaVersion: 1,
		...
		if (!response.ok) {
			...
			throw new CrmProviderAuthorizationError(
