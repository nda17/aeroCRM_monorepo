...
import { BillingCrmCommerceController } from './billing-crm-commerce.controller';
import {
	BillingCrmProviderController,
	RetryCrmProviderDto
} from './billing-crm-provider.controller';
import {
	CrmCheckoutDto,
	CrmHistoryDto,
	CrmVerifyOrderDto
...
describe('aeroCRM commerce isolated HTTP contracts', () => {
	it('excludes every real commerce POST from the production bootstrap public prefix, without wildcards', () => {
		...
		const expected = Object.getOwnPropertyNames(
			BillingCrmCommerceController.prototype
		)
			.flatMap(name => {
				const method = (
					BillingCrmCommerceController.prototype as unknown as Record<
						string,
						unknown
					>
				)[name];
	...
	it('requires the existing scoped Access pair on every commerce endpoint', () => {
		expect(
			Reflect.getMetadata(PATH_METADATA, BillingCrmCommerceController)
		).toBe('internal/v1/crm-access/billing/commerce');
		expect(
			Reflect.getMetadata(GUARDS_METADATA, BillingCrmCommerceController)
	...
	it.each([
		...
		await expect(
			pipe.transform(
				{ ...checkout, ...patch },
				{ type: 'body', metatype: CrmCheckoutDto }
			)
		).rejects.toBeDefined();
	});
	it('does not silently coerce the internal monetary command', async () =>
		expect(
			await pipe.transform(checkout, {
				type: 'body',
				metatype: CrmCheckoutDto
			})
		).toEqual(checkout));
	it('rejects mismatch before business work', () => {
		const service = { checkout: jest.fn() };
		const controller = new BillingCrmCommerceController(
			service as never
		);
		expect(() =>
	...
	it.each([
		...
		expect(
			pipe.transform(
				...
				{ type: 'body', metatype: CrmHistoryDto }
			)
		).rejects.toBeDefined()
	);
	it('verification has no caller-supplied provider ID or POST mode', async () =>
		expect(
			pipe.transform(
				...
				{ type: 'body', metatype: CrmVerifyOrderDto }
			)
		).rejects.toBeDefined());
	it('independent provider retry is DEV-only, not workspace-owner or ADMIN', () => {
		const retry = BillingCrmProviderController.prototype.retry;
	...
	it.each([
		...
		const retryPipe = (
			Reflect.getMetadata(
				PIPES_METADATA,
				BillingCrmProviderController.prototype.retry
			) as ValidationPipe[]
		)[0];
		await expect(
			retryPipe.transform(
				{ schemaVersion: 1, commandId: C, expectedVersion: 1, ...patch },
				{ type: 'body', metatype: RetryCrmProviderDto }
			)
		).rejects.toBeDefined();
	});
});
