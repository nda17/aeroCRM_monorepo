import { CrmCommerceSchedulerService } from './crm-commerce-scheduler.service';

jest.mock('../domain/crm-commerce.service', () => ({
	CrmCommerceService: class {}
}));

describe('aeroCRM durable renewal scheduler', () => {
	const originalFlag = process.env.BILLING_CRM_PAYMENTS_ENABLED;
	...
	let scheduler: CrmCommerceSchedulerService;
	...
	beforeEach(() => {
		...
		scheduler = new CrmCommerceSchedulerService(
	...
	afterEach(() => {
		...
		if (originalFlag === undefined)
			delete process.env.BILLING_CRM_PAYMENTS_ENABLED;
		else process.env.BILLING_CRM_PAYMENTS_ENABLED = originalFlag;
		...
		if (originalBrokerUrl === undefined)
			...
			process.env.BILLING_CRM_PROVIDER_RABBITMQ_URL = originalBrokerUrl;
	});
	...
	it.each(['yes', '1', 'on'])(
		...
		value => {
			process.env.BILLING_CRM_RECONCILIATION_ENABLED = value;
