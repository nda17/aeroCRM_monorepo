...
const settlement = {
	...
	salesFencedAt: acknowledgement.fencedAt
};

describe('CRM Intake workspace closure HTTP contract', () => {
	let app: NestExpressApplication;
	let origin: string;
	const fence = jest.fn();
	const settle = jest.fn();
	const post = (path: string, body: unknown) =>
		fetch(`${origin}/internal/v1/workspace-closures/${path}`, {
			method: 'POST',
	...
	beforeAll(async () => {
		const module = await Test.createTestingModule({
			...
			providers: [
				{ provide: WorkspaceClosureService, useValue: { fence, settle } }
			]
		})
		...
		app.setGlobalPrefix('api/v1', {
			exclude: [
				...
				{ path: 'internal/v1/workspace-closures/settle', method: RequestMethod.POST }
			]
		});
