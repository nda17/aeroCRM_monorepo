...
Надёжная очередь `aerocrm.reporting.settings` сохраняет имя, но её
единственным source binding является
`operations.notification-routing.changed.v1`. Retry-очереди
`aerocrm.reporting.settings.retry.1`, `.retry.2` и `.retry.3` возвращают
...
- `GET /internal/v1/reporting/messaging/overview` — доступный только через
  ...
  `REPORTING_OPERATIONS_TOKEN` и `x-aerocrm-service: operations`.
