...
describe('ReportingDeliveryFailuresService', () => {
	function harness(
		...
		const failure = {
			id: FAILURE_ID,
			eventId: EVENT_ID,
			consumer: input.consumer ?? 'reporting-crm-order-v1',
			consumerKind: input.consumerKind ?? 'crmOrder',
			eventType: input.eventType ?? 'billing.crm-order.succeeded.v1',
			payload,
			payloadHash,
			status,
			manualRetryCount:
	...
	it('creates the manual retry and audit Outbox rows in one transaction', async () => {
		...
		await expect(
			service.retryFailure(EVENT_ID, 'crmOrder', 'admin-1')
		).resolves.toEqual(
			expect.objectContaining({
				eventId: EVENT_ID,
				consumerKind: 'crmOrder',
