...
export class ReportingMetricsService {
	...
	render(): string {
		const lines = [
			'# HELP aerocrm_reporting_info Reporting process information.',
			'# TYPE aerocrm_reporting_info gauge',
			'aerocrm_reporting_info 1'
		];
		for (const [name, value] of [...this.counters].sort()) {
			lines.push(`# TYPE aerocrm_reporting_${name} counter`);
			lines.push(`aerocrm_reporting_${name} ${value}`);
		}
		for (const [name, value] of [...this.gauges].sort()) {
			lines.push(`# TYPE aerocrm_reporting_${name} gauge`);
			lines.push(`aerocrm_reporting_${name} ${value}`);
		}
		return `${lines.join('\n')}\n`;
	}
