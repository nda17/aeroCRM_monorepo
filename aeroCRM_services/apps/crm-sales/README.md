...
## Товары, услуги и расчёты

Дополнительные маршруты `/api/v1/crm/sales/commerce` обслуживают каталог рабочего
пространства, управление воронками, состав сделки, КП и ручные клиентские оплаты.
Старые ответы сделок, воронок, timeline и экспорта сохраняют свои строгие контракты.
Первоначальная установка шаблона остаётся отдельной идемпотентной операцией с
уникальным `PipelineTemplateInstallation.workspaceId`; новые воронки её не меняют.
Пустая воронка получает базовые OPEN/WON/LOST этапы. Переименование и порядок не
пересоздают сделки; удаления этапов и изменения их состояния API не предоставляет.

| Маршруты внутри commerce | Разрешение |
| --- | --- |
| `GET pipelines`, `GET catalog`, строки, КП, оплаты и история | `sales:read` |
| Создание/переименование воронок, добавление/переименование/порядок этапов | `sales:manage-pipelines`, OWNER/CRM_ADMIN |
| Изменение каталога/состава, сохранение строки, КП, оплаты/исправления, import preview/apply | `sales:write` |
| `GET analytics`, `GET analytics/pipelines` | `sales:analytics` |
| `GET export?format=json\|csv` | OWNER и `sales:export` |

Каждый запрос проходит актуальную workspace-авторизацию. Объекты сделки дополнительно
ограничены её текущим OWN/TEAM/ALL scope; каталог сам по себе не открывает чужие сделки.
READ_ONLY запрещает бизнес-запись. Команды содержат `schemaVersion:1`, workspaceId
и UUIDv4 commandId, совпадающий с `Idempotency-Key`. Receipt связан с автором,
workspace и нормализованным запросом. После потери ответа повтор той же команды
возвращает прежний результат, а повторное использование ключа с другими данными
отклоняется. История commerce отдельная: старый enum timeline не расширяется.

Каталог содержит стабильный уникальный внутри workspace код, тип PRODUCT/SERVICE,
название, единицу, nullable базовую цену, архивность и версию. Отсутствующая цена
не означает ноль. Строка сделки хранит собственный снимок и допускает явную нулевую
цену. Изменение/архивирование каталога и повторный импорт не переписывают эти снимки.
Разовая строка попадает в каталог только через явную команду; повторное сохранение
не создаёт дубль, конфликт кода не перезаписывает существующую позицию.

Деньги остаются Int в копейках: 0…2 147 483 647 (21 474 836,47 руб.). Количество —
положительный точный decimal до трёх знаков, максимум 999 999 999,999. Сервер
вычисляет gross через целочисленные тысячные и ROUND_HALF_UP до копейки, затем
вычитает денежную скидку. Скидка не больше gross; предел проверяется у строки и
всей сделки до записи. Замена не более 100 строк атомарна, требует expectedVersion
сделки и записывает before/after историю. До строк действует ручная сумма; после
строк — только расчётная. Пустой состав переключает на ручную сумму, явно переданную
командой, или на 0. Старая ручная сумма не восстанавливается.

Импорт: `POST import/inspect`, `import/preview`, `import/apply`. UTF-8 CSV допускает
запятую или точку с запятой; XLSX — выбор листа и текстовые артикулы с ведущими нулями.
Размер исходного файла до 1 000 000 байт, распакованного XLSX до 4 000 000 байт,
отдельной записи ZIP до 2 000 000 байт, до 100 записей ZIP, 20 листов и 500 строк
каталога. Формулы, макросы, внешние ссылки и неоднозначные коды отклоняются.
Preview сохраняет нормализованные строки, ошибки, версии и workspace/автора на
30 минут. Apply проверяет права заново и применяет именно этот снимок целиком.
Изменение каталога после preview даёт конфликт без частичной записи. Пустая цена
при обновлении сохраняет прежнюю, отсутствующие коды не архивируются, архивная
позиция не восстанавливается. Совпадение названий не используется для обновления.

КП создаётся из сохранённых строк: сведения о продавце и клиенте, суммы и версия
сделки фиксируются в неизменяемом снимке. Скачивание возвращает самостоятельный
HTML с экранированным текстом; его можно распечатать в PDF. Нет автоматической
отправки, электронной подписи или конструктора документов.

Поступления и возвраты не связаны с SaaS Billing. Исправление добавляет отменяющую
и заменяющую записи с автором и причиной; исходная запись остаётся. Эффективный
журнал исключает исправленные операции, не допускает отрицательного остатка на
любую дату и переполнения. Возврат не может предшествовать покрывающему поступлению.
Частичная оплата, долг и переплата видны независимо от стадии. Оплата не меняет
стадию, WON не создаёт оплату; существующее завершение открытых задач при WON/LOST
сохраняется. Стоимость сделок отбирается по созданию сделки, денежные операции —
по occurredAt; стоимость выигранных сделок не обозначается полученной выручкой.

Отдельный экспорт каталога, строк и полного журнала оплат доступен в JSON/CSV,
включает архивные записи и исправления, сохраняет прежний экспорт без изменений.
Лимиты: 10 000 сделок, 10 000 позиций каталога, 50 000 строк каталога/состава/оплат
суммарно и 16 МиБ. CSV экранирует формулы; это не шаблон обратного импорта.

Миграция `20260923010000_sales_commerce` аддитивна и сохраняет существующие ID и
ручные суммы. Runtime имеет только SELECT/INSERT на КП, оплаты, receipts и историю;
SQL-триггеры также запрещают UPDATE/DELETE/TRUNCATE этих данных. `database-access.json`
и Operations backup inventory обновляются вместе со схемой. Сначала выпускается
backend с выделенным `crm_sales_commerce_migration` hook под release lock, затем
frontend. Точный DDL/ACL hash, migration env hash и штатные gates обязательны.
Откат к старым writers после появления commerce-данных блокируется; удалять новые
данные ради отката нельзя. Порядок и восстановление описаны в infra README;
автоматический S3 restore остаётся выключенным до отдельной приёмки.

Проверки: обычные `pnpm test`, `pnpm lint`, `pnpm typecheck`, `pnpm build`; CI gate
`crm-sales-commerce-postgres18` дополнительно применяет миграции и ACL в изолированной
PostgreSQL 18 и запускает `test/integration/sales-commerce-postgres18.integration.mjs`
под ограниченной runtime-ролью. Этот тест разрешает только loopback test database
с явным `CRM_SALES_INTEGRATION_ALLOW_MUTATION=true`; он не загружает production env.
...
`aerocrm.crm.sales.reminders.dead-letter`. Runtime не объявляет topology.
