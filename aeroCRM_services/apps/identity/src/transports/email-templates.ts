...
function layout(input: {
	...
	const siteUrl =
		process.env.TURNSTILE_CLIENT_URL || 'https://aerocrm.space';
...
export function verificationEmail(code: string) {
	return layout({
		preview: 'Подтвердите email в aeroCRM',
		title: '🔐 Код подтверждения email',
		subtitle: 'Подтвердите адрес электронной почты для входа в aeroCRM',
...
export function passwordEmail(password: string) {
	const siteUrl =
		process.env.TURNSTILE_CLIENT_URL || 'https://aerocrm.space';
	return layout({
		preview: 'Ваш временный пароль для входа в aeroCRM',
