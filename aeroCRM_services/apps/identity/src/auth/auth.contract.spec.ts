...
describe('password login and revocation row-lock contract', () => {
	it('rejects a stale password snapshot when the password mutation locks first', async () => {
		...
		const loginLookupStarted = deferred();
		const continueLogin = deferred();
		const staleUser = value.currentUser();
		value.users.findByIdentity.mockImplementation(async () => {
			loginLookupStarted.resolve();
		...
		await loginLookupStarted.promise;
