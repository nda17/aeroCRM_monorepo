...
export class IdentityInternalService {
			...
			try {
				...
				if (attempt >= 2 || !(error instanceof Prisma.PrismaClientKnownRequestError &&
					(error.code === 'P2034' || (error.code === 'P2010' &&
						['40001', '40P01'].includes(String(error.meta?.code)))))) throw error;
			}
		}
	}
	...
	/** Canonical CRM owner and delegated actor come from one Identity snapshot. */
	async crmOwnerContext(workspaceId: string, subject: string) {
		const denied = {
			schemaVersion: 1 as const,
			workspaceId,
			subject,
