...
export class AvatarStorageService {
	...
	constructor(config: ConfigService) {
		...
		if (
			...
			(endpoint.protocol !== 'https:' ||
				this.publicBaseUrl.toString().replace(/\/$/, '') !==
					'https://api.aerocrm.space/api/v1/users/avatar-files')
		) {
	...
	async readActiveObject(objectKey: string): Promise<{ body: Buffer; contentType: AvatarMimeType }> {
		parseAvatarObjectKey(objectKey);
		try {
			...
			if (
				!object.Body ||
				!AVATAR_MIME_TYPES.includes(object.ContentType as AvatarMimeType) ||
				!Number.isSafeInteger(object.ContentLength) ||
				object.ContentLength! < 1 ||
				object.ContentLength! > AVATAR_MAX_UPLOAD_BYTES
			) {
			...
			for await (const chunk of object.Body as AsyncIterable<Uint8Array>) {
				...
				size += part.length;
				if (size > AVATAR_MAX_UPLOAD_BYTES) throw new NotFoundException('Avatar not found');
				chunks.push(part);
			}
			if (size !== object.ContentLength) throw new NotFoundException('Avatar not found');
			const body = Buffer.concat(chunks, size);
			if (detectAvatarMime(body) !== object.ContentType) throw new NotFoundException('Avatar not found');
			return { body, contentType: object.ContentType as AvatarMimeType };
		} catch (error) {
			if (error instanceof NotFoundException || (error as { name?: string }).name === 'NoSuchKey') {
				throw new NotFoundException('Avatar not found');
			}
			throw new ServiceUnavailableException('Avatar storage is temporarily unavailable');
		}
	}
