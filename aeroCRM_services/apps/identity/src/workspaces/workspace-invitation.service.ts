...
export class WorkspaceInvitationService {
	...
	constructor(
		...
		const enabled =
			config.get<string>('CRM_INVITATION_EMAIL_ENABLED')?.trim() ||
			'false';
		if (!['true', 'false'].includes(enabled))
			throw new Error(
				'CRM_INVITATION_EMAIL_ENABLED must be true or false'
			);
	...
	async acceptanceContext(invitationId: string, workspaceId: string) {
		const invitation = await this.prisma.workspaceInvitation.findFirst({
			where: { id: invitationId, workspaceId, productCode: 'AEROCRM' }
		});
		if (!invitation) throw new NotFoundException('Invitation not found');
