...
describe('deal views and server pipeline', () => {
	it('opens the deal once from a non-title cell and preserves native title-button activation', async () => {
		render(view())
		await ready()
		const stage = screen
			.getAllByText('Новая')
		...
		expect(
			screen.getAllByRole('button', { name: 'Закрыть сделку' })
		).toHaveLength(1)
		...
		const title = screen.getByRole('button', {
			name: 'Новый заказ Клиент'
		})
		expect(title.getAttribute('type')).toBe('button')
		// detail=0 models the click generated by native keyboard activation.
